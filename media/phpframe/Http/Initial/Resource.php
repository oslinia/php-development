...
class Resource extends Frame
{
    ...
    private function resource_media(string $dirname): void
    {
        is_dir($dirname) || mkdir($dirname, recursive: true);

        $filename = $dirname . '/style.css';

        is_file($filename) || file_put_contents($filename, '@charset "UTF-8";' . PHP_EOL . PHP_EOL .
            '* {' . PHP_EOL .
            '    margin: 0;' . PHP_EOL .
            '    padding: 0;' . PHP_EOL .
            '}' . PHP_EOL);
    ...
    public function __construct(string $filename)
    {
        ...
        $this->resource_media(parent::root('resource', 'media', 'css'));
